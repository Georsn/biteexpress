...
import { CheckCircle2, Flame, Truck, CheckSquare, Clock, MapPin, CreditCard, ChevronRight, Activity } from 'lucide-react';
...
export default function OrderTracking({
  ...
  if (!order) {
    return (
      <div id="no-order-active" className="p-5 text-white flex flex-col items-center justify-center text-center gap-4 py-20">
    ...
    <div id="tracking-view" className="p-4 flex flex-col gap-5 text-white select-none pb-10">
      ...
      <div id="simulation-panel-card" className="p-4 rounded-3xl bg-neutral-900/65 border border-dashed border-neutral-800 flex flex-col gap-3">
        <div className="flex flex-col gap-1 border-b border-neutral-850 pb-2">
          <span className="text-[10px] text-orange-500 font-extrabold uppercase tracking-wide">Painel Universitário de Simulação</span>
          ...
          {stepsList.map((step) => {
            ...
            return (
              ...
              >
                ...
                <span>{step.id === 'received' ? 'Receber' : step.id === 'preparing' ? 'Preparar' : step.id === 'delivery' ? 'Enviar' : 'Entregar'}</span>
              </button>
            );
          })}
        </div>
